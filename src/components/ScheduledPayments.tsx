...
export const ScheduledPayments: React.FC<ScheduledPaymentsProps> = ({
  ...
  const handleProcessDuePayments = async () => {
    ...
    const confirmMessage = `You are about to process ${duePayments.length} payment${duePayments.length > 1 ? 's' : ''}:\n\n${paymentList}\n\nTotal: $${totalAmount.toLocaleString()} XLM\n\n⚠️ Your Stellar wallet will open ${duePayments.length > 1 ? `${duePayments.length} popup${duePayments.length > 1 ? 's' : ''}` : 'a popup'} for confirmation. You'll need to approve each transaction.\n\nProceed?`;
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-8">
        <div className="bg-white border border-gray-200 rounded-lg p-4 sm:p-8 shadow-sm">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="text-purple-800 font-medium">
                      Pre-approved Spending Limit: ${preApprovalLimit.toLocaleString()} XLM
                    </p>
          ...
          {duePayments.length > 0 && (() => {
            ...
            return (
              <div className={`mb-6 p-4 border rounded-lg ${isWithinLimit
                    ...
                    {isWithinLimit ? (
                      ...
                      <>
                        <p className="text-yellow-700 text-sm mt-1">
                          ⚠️ Total amount (${totalDueAmount.toLocaleString()} XLM) exceeds your pre-approved limit (${preApprovalLimit?.toLocaleString() || 0} XLM). Click "Process Due Payments" to execute manually.
                        </p>
                  ...
                  <div className="mb-4">
                    ...
                    {preApprovalLimit !== null && preApprovalLimit > 0 && (
                      <p className="text-xs text-gray-500 mt-1">
                        Current limit: ${preApprovalLimit.toLocaleString()} XLM
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex space-x-3">
                    ...
                    <button
                      onClick={async () => {
                        ...
                        alert(`Pre-approval limit set to $${limit.toLocaleString()} XLM. Payments within this limit will be automatically triggered.`);
